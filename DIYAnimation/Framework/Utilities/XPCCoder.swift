...
/* TODO: Support MachSend/ReceiveRight, AuditToken, SharedMemory, and IOSurface. */
    ...
    private class KeyedContainer<Key: CodingKey>: KeyedEncodingContainerProtocol, EncodingChildContainer {
        ...
        func encode<T: Encodable>(_ value: T, forKey key: Key) throws {
            if value is Date {
                ...
                #if canImport(IOSurface)
                if value is IOSurface {
                    try self.encode(value as! IOSurface, forKey: key)
                    return // swift #if is a bit weird?
                }
                #endif
                try key.stringValue.withCString {
                    try throwIfExists(key, $0, value)
